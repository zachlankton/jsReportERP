...
function autoTextArea(options){
    ...
    var autoList = JSON.stringify( options.data.root[args.list] )
    return new Handlebars.SafeString(`
    <p class="w3-autocomplete">
        <label> ${args.label} </label>
        <textarea   id="${id}" autocomplete="nope"
                    class="w3-input w3-border w3-round w3-padding-small" 
                    style="resize:none; height:150px;"
                    name="${args.name}" 
                >${this[args.name] || ""}</textarea>
        <script>
            autocomplete(document.getElementById("${id}"), ${autoList})
        </script>
    </p>
    `);
}
